    ...
    const float *angles, unsigned int size, SDL_bool is_one_src)
{
    ...
    if (!texture) {
        SDL_SetError("SDL_CreateBatch: Texture is null");
        goto error;
    }
    if (!size) {
        SDL_SetError("SDL_CreateBatch: size is 0");
        goto error;
    }
    ...
    batch->sources = SDL_malloc(size * sizeof(SDL_Rect));
    ...
    SDL_SetBatchAngles(renderer, batch, angles);
    SDL_SetBatchDestinations(renderer, batch, dstrect);
    SDL_SetBatchColors(renderer, batch, colors);
...
void SDL_DestroyBatch(SDL_Renderer *renderer, SDL_Batch *batch)
{
    if (batch)
    {
...
int SDL_GetBatchSize(SDL_Batch* batch)
{
    if (batch == NULL)
        return 0;
...
void SDL_QueryBatchDestinations(SDL_Batch* batch, SDL_Rect** dst, int* size)
{
    if (batch == NULL || dst == NULL)
        return;

    *dst = batch->destinations;
    if (size)
        *size = batch->size;
}

void SDL_QueryBatchAngles(SDL_Batch* batch, float** angles, int* size)
{
    if (batch == NULL || angles == NULL)
        return;
...
void SDL_QueryBatchColors(SDL_Batch* batch, SDL_Color** colors, int* size)
{
    if (batch == NULL || colors == NULL)
        return;
