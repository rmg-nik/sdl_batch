...
static int GL_CreateBatch(SDL_Renderer *renderer, SDL_Batch *batch, SDL_Texture *texture, const SDL_Rect *srcrect, const SDL_Rect *dstrect,
    const SDL_Color *colors, const float *angles, unsigned int size, SDL_bool is_one_src);
static void GL_DestroyBatch(SDL_Renderer *renderer, SDL_Batch *batch);
static int GL_RenderBatch(SDL_Renderer *renderer, SDL_Batch *batch);
static void GL_SetBatchDestinations(SDL_Batch *batch, const SDL_Rect *dstrect);
static void GL_SetBatchColors(SDL_Batch *batch, const SDL_Color *colors);
static void GL_SetBatchAngles(SDL_Batch *batch, const float *angles);
static void UpdateBatchNativeDataPositions(SDL_Batch *batch);
static void UpdateBatchNativeDataColors(SDL_Batch *batch);
...
/*
static int
GL_CreateBatch(SDL_Renderer *renderer, SDL_Batch *batch, SDL_Texture *texture, const SDL_Rect *srcrect, const SDL_Rect *dstrect,
    const SDL_Color *colors, const float *angles, unsigned int size, SDL_bool is_one_src)
{
    if (!texture)
    {
        SDL_SetError("GL_CreateBatch: Texture is null");
        return -1;
    }
    if (!size)
    {
        SDL_SetError("GL_CreateBatch: size is 0");
        return -1;
    }

    //If cannot allocate enough memory - release resources after return -1 in SDL_CreateBatch via call DestroyBatch
    batch->native_data = SDL_malloc(sizeof(GL_Batch));
    if (batch->native_data == NULL)
    {
        SDL_SetError("GL_CreateBatch: Cannot allocate enough memory for batch");
        return -1;
    }

    GL_Batch* native_data = (GL_Batch*)batch->native_data;
    native_data->verticesColor = native_data->verticesPosition = native_data->verticesTexture = NULL;

    native_data->verticesPosition = SDL_malloc(sizeof(GLfloat) * size * 2);
    if (native_data->verticesPosition == NULL)
    {
        SDL_SetError("GL_CreateBatch: Cannot allocate enough memory for batch");
        return -1;
    }
    native_data->verticesColor = SDL_malloc(sizeof(GLfloat) * size * 4);
    if (native_data->verticesColor == NULL)
    {
        SDL_SetError("GL_CreateBatch: Cannot allocate enough memory for batch");
        return -1;
    }
    native_data->verticesTexture = SDL_malloc(sizeof(GLfloat) * size * 2);
    if (native_data->verticesTexture == NULL)
    {
        SDL_SetError("GL_CreateBatch: Cannot allocate enough memory for batch");
        return -1;
    }

    batch->destinations = SDL_malloc(sizeof(SDL_Rect) * size);
    if (batch->destinations == NULL)
    {
        SDL_SetError("GL_CreateBatch: Cannot allocate enough memory for batch");
        return -1;
    }

    batch->colors = SDL_malloc(sizeof(SDL_Color) * size);
    ...
    batch->angles = SDL_malloc(sizeof(float) * size);
    if (batch->angles == NULL)
    {
        SDL_SetError("GL_CreateBatch: Cannot allocate enough memory for batch");
        return -1;
    }
    ...
    batch->size = size;
    batch->is_one_src = is_one_src;
    batch->texture = texture;    

    //Set texture coords. They cannot change in the future.
    float minu, maxu, minv, maxv;

    if (!srcrect || is_one_src == SDL_TRUE)
    {
        minu = 0.0f;
        maxu = 1.0f;
        minv = 0.0f;
    ...
    GLfloat* vertices = native_data->verticesTexture;
    unsigned int i = 0;
    for (; i < batch->size; ++i)
    {
        if (srcrect && is_one_src == SDL_FALSE)
        {
        ...
        vertices[0 + i * 6 + 0] = minu;
        vertices[0 + i * 6 + 1] = minv;

        vertices[1 + i * 6 + 0] = maxu;
        vertices[1 + i * 6 + 1] = minv;

        vertices[2 + i * 6 + 0] = maxu;
        vertices[2 + i * 6 + 1] = maxv;

        vertices[3 + i * 6 + 0] = maxu;
        vertices[3 + i * 6 + 1] = maxv;

        vertices[4 + i * 6 + 0] = minu;
        vertices[4 + i * 6 + 1] = maxv;

        vertices[5 + i * 6 + 0] = minu;
        vertices[5 + i * 6 + 1] = minv;
    }

    glGenVertexArrays(1, &native_data->VAO);
    glBindVertexArray(native_data->VAO);


    glGenBuffers(1, &native_data->VBOPosition);
    glBindBuffer(GL_ARRAY_BUFFER, native_data->VBOPosition);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * size * 2, native_data->verticesPosition, GL_STATIC_DRAW);

    const GLint vertexPosition = 0;
    glVertexAttribPointer(vertexPosition, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), (GLvoid*)0);
    glEnableVertexAttribArray(vertexPosition);

    glGenBuffers(1, &native_data->VBOTexture);
    glBindBuffer(GL_ARRAY_BUFFER, native_data->VBOTexture);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * size * 2, native_data->verticesTexture, GL_STATIC_DRAW);

    const GLint vertexTexture = 1;
    glVertexAttribPointer(vertexTexture, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), (GLvoid*)0);
    glEnableVertexAttribArray(vertexTexture);

    glGenBuffers(1, &native_data->VBOColor);
    glBindBuffer(GL_ARRAY_BUFFER, native_data->VBOColor);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * size * 4, native_data->verticesColor, GL_STATIC_DRAW);

    const GLint vertexColor = 3;
    glVertexAttribPointer(vertexColor, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (GLvoid*)0);
    glEnableVertexAttribArray(vertexColor);

    glBindVertexArray(0);

    D3D_SetBatchAngles(batch, angles);
    D3D_SetBatchDestinations(batch, dstrect);
    D3D_SetBatchColors(batch, colors);
    return 0;
}
...
{
    if (batch->native_data)
        SDL_free(batch->native_data);

    if (batch->destinations)
        SDL_free(batch->destinations);

    if (batch->colors)
        SDL_free(batch->colors);

    if (batch->angles)
        SDL_free(batch->angles);
...
GL_RenderBatch(SDL_Renderer *renderer, SDL_Batch *batch)
{
    ...
    if (batch->need_update_colors)
        UpdateBatchNativeDataColors(batch);
    if (batch->need_update_positions)
        UpdateBatchNativeDataPositions(batch);

    Vertex* vertices = (Vertex*)batch->native_data;
...
static void D3D_SetBatchDestinations(SDL_Batch *batch, const SDL_Rect *dstrect)
{
    if (dstrect == NULL)
    {
        SDL_SetError("D3D_UpdateBatchDestinations: Destination is NULL");
        return;
    }

    batch->need_update_positions = SDL_TRUE;
    ...
    for (unsigned int i = 0; i < batch->size; ++i)
    {
        ...
        batch_dst->y = dstrect->y;
        batch_dst->w = dstrect->w;
        batch_dst->h = dstrect->h;
        ++dstrect;
        ++batch_dst;
    }
}

static void D3D_SetBatchColors(SDL_Batch *batch, const SDL_Color *colors)
{
    batch->need_update_colors = SDL_TRUE;
    ...
    for (unsigned int i = 0; i < batch->size; ++i)
    {
        ...
        {
            batch_colors->r = texture->r;
            batch_colors->g = texture->g;
            batch_colors->b = texture->b;
            batch_colors->a = texture->a;
        }
...
static void D3D_SetBatchAngles(SDL_Batch *batch, const float *angles)
{
    batch->need_update_positions = SDL_TRUE;
...
static void UpdateBatchNativeDataPositions(SDL_Batch* batch)
{
    batch->need_update_positions = SDL_FALSE;
...
static void UpdateBatchNativeDataColors(SDL_Batch* batch)
{
    batch->need_update_colors = SDL_FALSE;
